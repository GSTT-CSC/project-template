    ...
    async def start_async_process(self):
        with ThreadPoolExecutor(max_workers=self.num_workers) as executor:
            with xnat.connect(server=self.xnat_configuration['server'],
                ...
                responses = [await f for f in tqdm.tqdm(asyncio.as_completed(tasks), total=len(tasks))]  # noqa: F841
                        ...
                        for obj in xnat_obj:
                            if obj.cache_id[0] == 'ResourceCatalog':
                                action_data.append({'source_action': action.__name__,
                                                    ...
                                                    'resource_files': list(obj.files.keys())})  # filenames (keys) so membership checks work on xnat>=0.8
                            else:
                                action_data.append({'source_action': action.__name__,
                                                    'action_data': obj.uri,
                                                    'data_type': 'xnat_uri',
...
def xnat_build_dataset(xnat_configuration: dict, actions: list = None, flatten_output=True, test_batch: int = -1):
    """
    ***NOTE THE ASYNC DataBuilderXNAT ABOVE WILL BE MUCH FASTER FOR LARGE DATASETS***

    Builds a list describing the XNAT project dataset using the XNAT data hierarchy:
    project/subject/experiment/scan. Returns a flattened dataset by default.
    """
